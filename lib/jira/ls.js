...
const lsCommand = (prog, extCallback) => {
    prog
        ...
        try {
            ...
            const meta = await helpers_1.client.issues.getCreateIssueMetadata();
...
async function lsEntry(options) {
    const e = options;
    return await helpers_1.client.issueSearch.searchForIssuesUsingJqlGet({ jql: 'assignee=currentUser()', fields: ['*all']
    });
...
function displayIssues(issues, options) {
    ...
    for (let i = 0; i < issues.length; i += 1) {
        let priority = issues[i].fields?.priority, summary = issues[i].fields.summary;
