...
exports.doneIssue = exports.stopIssue = exports.startIssue = exports.registerTransitionCommand = void 0;
...
const startIssue = (issue, cb) => transitionIssue(issue, cb, 'jira_start');
exports.startIssue = startIssue;
const stopIssue = (issue, cb) => transitionIssue(issue, cb, 'jira_stop');
exports.stopIssue = stopIssue;
const doneIssue = (issue, cb) => transitionIssue(issue, cb, 'jira_done');
exports.doneIssue = doneIssue;
const transitionIssue = async (issue, cb, transitionType) => {
    const startTransitionName = config_1.default.options[transitionType]['status'];
    const transitions = await helpers_1.client.issues.getTransitions({
        issueIdOrKey: issue,
    ...
    try {
        const response = await helpers_1.client.issues.transitionIssue({ issueIdOrKey: issue, transition: { id: newTransition.id } });
        cb(response, null);
    }
    catch (e) {
        cb(null, e);
