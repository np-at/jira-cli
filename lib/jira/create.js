...
const DynamicPrompt_1 = require("../helpers/DynamicPrompt");
...
const assembleCreationParameters = async (options) => {
    ...
    const e = {};
    await DynamicPrompt_1.dynamicPrompt('project', e, userConfigPrefs);
    await DynamicPrompt_1.dynamicPrompt('issueType', e, userConfigPrefs);
    if (e['issueType']?.subtask === true)
        await DynamicPrompt_1.dynamicPrompt('parentTask', e, userConfigPrefs);
    await DynamicPrompt_1.dynamicPrompt('details', e, userConfigPrefs);
    const requestFieldsObject = {
        project: {
            id: e['project'].id
        },
        assignee: {
            name: (await helpers_1.client.myself.getCurrentUser())['name']
        },
        summary: e['summary'],
        issuetype: {
            id: e['issueType'].id
...
function creation(jira) {
    const create = {
        ...
        ask: function (question, callback, yesno, values, answer) {
            let that = this, options = {}, issueTypes = [], i = 0;
            ...
            inquirer_1.default.prompt({ message: question, name: question }, function (answer) {
                if (answer.length > 0) {
                    callback(answer);
                }
                else {
