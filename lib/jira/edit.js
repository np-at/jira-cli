...
const editIssue = async (issueInput) => {
    let issue;
    try {
        issue = await helpers_1.client.issues.getEditIssueMetadata({ issueIdOrKey: issueInput });
    }
    catch (e) {
        console.error(e);
        throw e;
...
exports.default = (() => {
    const editMeta = {
        ask: function (question, callback, yesno, values, answer) {
            ...
            let match;
            inquirer_1.prompt({ message: question }).then((answer) => {
                debugger;
                if (answer.length > 0) {
