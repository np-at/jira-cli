...
const projectPrompt = async (currentAnswers, defaultProject) => {
    const projects = await helpers_1.client.projects.getAllProjects({ expand: 'issueTypes' });
...
const parentTaskPrompt = async (currentAsnwers, userPreferences) => {
    if (!currentAsnwers.project)
