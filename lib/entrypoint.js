...
exports.default = (async () => {
    ...
    const program = new commander_1.Command().enablePositionalOptions(true).storeOptionsAsProperties(false).allowUnknownOption(true).allowExcessArguments(true);
    ...
    program
        ...
        try {
            const issueSuggestions = await helpers_1.client.issueSearch.getIssuePickerSuggestions({ currentJQL: 'assignee=currentUser()' });
    ...
    program
        ...
        try {
            ...
            const issueSuggestions = await helpers_1.client.issueSearch.getIssuePickerSuggestions({ query: '' });
            console.log(issueSuggestions.sections.flatMap(x => x.issues).map(x => String(`${x.key}|*|${x.summary}`)).join(os.EOL));
            return;
        }
        catch (e) {
